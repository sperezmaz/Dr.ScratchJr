...
def over_repeat2(sequence, sequence_onm):
    """Function looks for a message of a color within a Repeat block.."""
    if sequence.count('repeat') == 1:
        for sequences_repeat in sequence:
            if isinstance(sequences_repeat, list):
                for sequence_repeat in sequences_repeat:
                    if sequence_repeat.count("message") == 1:
                        if sequence_onm[1] == sequence_repeat[1]:
                            return True
                    if over_repeat2(sequence_repeat, sequence_onm):
                        return True
                                    ...
                                    for sequence1 in (adapted_dict[pages]
                                        ...
                                        if sequence1.count("message") == 1:
                                            if sequence[1] == sequence1[1]:
                                                found = True
                                        if over_repeat2(sequence1, sequence):
                                            found = True
                            if not found:
                                triggering_found = False
